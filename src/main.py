...
trainset = MyDataSet(root="/data3/jdq/imgc/",max_number_class=number_of_class, transform=transforms.Compose([
    transforms.Resize((112, 96)),
    transforms.ToTensor(),
...
evalset = LfwDataSet(root="/data3/jdq/lfwc/",pairfile="../pair.txt",transform=transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))]))
eval_loader = torch.utils.data.DataLoader(evalset, batch_size=1, shuffle=True, num_workers=16)
# Model
print('==> Building model..')
#model=None
if mainloss=='A-softmax':
    model = sphere20a(linear='AngleLinear',classnum=number_of_class)
elif mainloss=='cosloss' or mainloss=='mcosloss' or mainloss=='arcloss':
    model = sphere20a(linear='CosLinear',classnum=number_of_class)
elif mainloss == 'cocoloss':
    model = sphere20a(linear='CocoLinear',scale=args.s,classnum=number_of_class)
elif mainloss == 'mcocoloss':
    model = sphere20a(linear='mCocoLinear',scale=args.s,classnum=number_of_class)
else: 
    model = sphere20a(classnum=number_of_class)
...
def eval(epoch):
    predicts = []
    model.eval()
    for batch_idx,(data1,data2, target) in enumerate(eval_loader):
        data1, data2, target = data1.to(device),data2.to(device) ,target.to(device)
        ip1, _ = model(data1)
        ip2, _ = model(data2)
        ip1=ip1.data.cpu()[0]
        ip2=ip2.data.cpu()[0]
        cosdistance = ip1.dot(ip2) / (ip1.norm() * ip2.norm() + 1e-5)
        predicts.append([cosdistance, target])
    ...
    accuracy.append(lfw.eval_acc(best_thresh, predicts))
