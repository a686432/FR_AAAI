...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000,linear='linear',scale=15):
        ...
        #     ])
        # )
        self.ip1 = nn.Linear(512*block.expansion, 512)
        # self.ip2 = nn.Linear(2, 10,bias=False)
        if linear=='AngleLinear':
            self.ip2 = AngleLinear(512, num_classes)
            print("Use AngleLinear")
        elif linear=='CosLinear':
            self.ip2 = CosLinear(512,num_classes)
            print("Use CosLinear")
        elif linear=='CocoLinear':
            self.ip2 = CocoLinear(512,num_classes,c=scale)
            print("Use CocoLinear")
        elif linear=='mCocoLinear':
            self.ip2 = mCocoLinear(512,num_classes,c=scale)
            print("Use CocoLinear")
        else:
            self.ip2 = nn.Linear(512, num_classes,bias=False)
...
def resnet50(pretrained=False, model_root=None, linear="linear",scale=15,num_classes=79077):
    model = ResNet(Bottleneck, [3, 4, 6, 3], linear=linear,scale=scale,num_classes=num_classes)
    # if pretrained:
